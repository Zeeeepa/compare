...
class GitHubCompare:
    ...
    def remove_commits_from_branch(self, repo, branch_name, selected_commits):
        """
        Remove selected commits from a branch.
        
        Args:
            repo: The GitHub repository object
            branch_name: Name of the branch to modify
            selected_commits: List of commit SHAs to remove
            
        Returns:
            The number of commits removed
            
        Raises:
            BranchNotFoundError: If the branch is not found
            CommitNotFoundError: If a commit is not found
            GitOperationError: If a git operation fails
            CommitRemovalError: For other commit removal errors
        """
        try:
            ...
            try:
                ...
                # Reset temporary branch to base commit
                try:
                    temp_ref.edit(base_commit.sha, force=True)
                except GithubException as e:
                    raise GitOperationError(f"Failed to reset temporary branch: {str(e)}")
                
                # Pre-compute valid commit SHAs for performance
                ...
                for i, commit in enumerate(reversed(commit_to_keep[:-1])):
                    try:
                        ...
                        # For merge commits, find the correct parent
                        if len(commit_data.parents) > 1:
                            # Find the parent that's in our keep list using the precomputed set
                ...
                try:
                    ...
                    raise GitOperationError(f"Failed to update branch: {str(e)}")
                
                return len(selected_commits)
        ...
        except CommitRemovalError:
            raise
        except Exception as e:
            raise CommitRemovalError(f"Unexpected error during commit removal: {str(e)}")
    ...
    def remove_selected_commits(self):
        ...
        def perform_removal():
            try:
                ...
                num_removed = self.remove_commits_from_branch(repo, branch_name, selected_commits)
                self.root.after(0, lambda: self.after_commit_removal(num_removed))
